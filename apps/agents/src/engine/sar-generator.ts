...
Additional context: ${input.additionalContext ?? 'None provided'}`,
    ...
    const draft = await prisma.sarDraft.create({
      data: {
        ...
        subjectInfo: (parsed['subjectInfo'] ?? {}) as Record<string, unknown>,
        activityDescription: String(parsed['activityDescription'] ?? ''),
        timeline: (parsed['timeline'] ?? []) as Record<string, unknown>[],
        evidenceCitations: (parsed['evidenceCitations'] ?? []) as Record<string, unknown>[],
        regulatoryRefs: (parsed['regulatoryRefs'] ?? []) as Record<string, unknown>[],
        filingDeadline,
